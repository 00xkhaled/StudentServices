...
public class AddEditRidesDao extends ConnectionDao {    
    ...
    public AvailableRide getRide(int rideId) throws Exception {
        ...
        try {   
            ...
            String sql = "select student_id, name, phone, gender, ride_from, ride_to, departure_time, " 
                    +"car_plate_nember, car_make, car_model, year_of_make, car_color"
                    +"from students_carpooling, destination, cars" 
                    +"where (students_carpooling.ride_id==destination.ride_id" 
                    +"and students_carpooling.ride_id==cars.ride_id and students_carpooling.ride_id=? );";                     
    ...
    public void insertRide(AvailableRide ride) throws Exception {
        ...
        try {
            ...
            sql = "insert into DESTINATION VALUES((select max(RIDE_ID) from STUDENTS_CARPOOLING)+1,?,?,?)";
            ps = conn.prepareStatement(sql);
